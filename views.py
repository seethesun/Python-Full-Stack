...
from firstapp.forms import CommentForm,UserForm
...
def myinfo(request):
    ...
    if request.user.is_authenticated():
        if request.method == "GET":
            #获取对应的User的额外信息，映射到表单上面初始化的数据
            form = UserForm({'realname':request.user.profile.realname,'sex':request.user.profile.sex,'avatar':request.user.profile.avatar})
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                # c =UserProfile(realname=realname,sex=sex,avatar=avatar)
                c =UserProfile(belong_to=request.user,realname=realname,sex=sex,avatar=avatar)
                c.save()

                u = User.objects.get(username=realname)
                print(type(u),'*'*20)
...
def mycollection(request):
    ...
    paget_robot = Paginator(article_list,3)
    page_num = request.GET.get('page')
    try:
        article_list = paget_robot.page(page_num)
    except EmptyPage :
        article_list = paget_robot(paget_robot.num_pages)
    except PageNotAnInteger :
        article_list = paget_robot.page(1)
